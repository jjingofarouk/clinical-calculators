...
// Lung Resection Risk Calculator Component
// Estimates risk of complications for lung resection surgery
// Based on Kozower et al. (Ann Thorac Surg 2008;86:366-373)
const LungResectionRisk = () => {
  ...
  const ranges = {
    ...
    dlco: { min: 20, max: 120, type: 'float', label: 'DLCO (% predicted)' },
    ...
    extent: { min: 1, max: 3, type: 'integer', label: 'Extent of Resection (1 = Wedge, 2 = Lobectomy, 3 = Pneumonectomy)' },
  };
  ...
  const helperText = {
    ...
    dlco: 'Diffusing capacity for carbon monoxide, % predicted (20-120%).',
    ...
    extent: 'Extent of resection: 1 = Wedge resection, 2 = Lobectomy, 3 = Pneumonectomy.',
  };
  ...
  const handleCalculate = () => {
    ...
    const coefficients = {
      age: formData.age > 65 ? (formData.age - 65) * 0.03 : 0,
      sex: formData.sex === '1' ? 0.2 : 0,
      fev1: formData.fev1 < 60 ? 0.5 : 0,
      dlco: formData.dlco < 60 ? 0.4 : 0,
      performanceStatus: formData.performanceStatus * 0.3,
      comorbidity: formData.comorbidity * 0.2,
      extent: [0, 0, 0.4, 0.8][formData.extent],
    };

    const sum = Object.values(coefficients).reduce((a, b) => a + b, -3.2);
  ...
  return (
    <Box className="min-h-screen w-full bg-background text-foreground p-2">
      <Typography variant="h4" className="font-bold text-foreground mb-4">
        Lung Resection Risk Calculator
      </Typography>
      <Box className="flex items-center mb-4">
        <Info className="w-5 h-5 text-primary mr-2" />
        <Typography variant="body2" className="text-muted-foreground">
          Estimates risk of complications for lung resection surgery. (Source: Kozower et al., Ann Thorac Surg 2008)
        </Typography>
      </Box>
          ...
          <Box className="mt-6 pt-4 border-t border-border">
            ...
            <Box className="flex items-center mt-2">
              ...
              <Typography variant="body2" className="text-muted-foreground">
                Source:{' '}
                <a
                  href="https://pubmed.ncbi.nlm.nih.gov/18640301/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-primary underline"
                >
                  Kozower et al., Ann Thorac Surg 2008;86:366-373
                </a>
              </Typography>
            </Box>
          </Box>
...
export default LungResectionRisk;