...
const SternalInfection = () => {
  ...
  const ranges = {
    ...
    obesity: { min: 0, max: 1, type: 'integer', label: 'Obesity (BMI > 30; 0 = No, 1 = Yes)' },
    smoking: { min: 0, max: 1, type: 'integer', label: 'Current Smoking (0 = No, 1 = Yes)' },
    surgeryDuration: { min: 1, max: 12, type: 'float', label: 'Surgery Duration (hours)' },
  };
  ...
  const helperText = {
    ...
    obesity: 'Body mass index > 30 kg/m² (0 = No, 1 = Yes).',
    smoking: 'Current smoking status (0 = No, 1 = Yes).',
    surgeryDuration: 'Duration of surgery in hours (1-12).',
  };
  ...
  const handleCalculate = () => {
    ...
    const coefficients = {
      age: formData.age > 70 ? 0.2 : 0,
      sex: formData.sex === '1' ? 0.1 : 0,
      diabetes: formData.diabetes === '1' ? 0.4 : 0,
      obesity: formData.obesity === '1' ? 0.3 : 0,
      smoking: formData.smoking === '1' ? 0.2 : 0,
      surgeryDuration: formData.surgeryDuration > 4 ? 0.1 * (formData.surgeryDuration - 4) : 0,
    };
    ...
    if (score < 2) {
      ...
      interpretation = 'Low risk of sternal infection; standard wound care recommended.';
    } else if (score <= 5) {
      riskLevel = 'Moderate Risk';
      riskColor = 'bg-yellow-100 text-yellow-800';
      interpretation = 'Moderate risk; consider enhanced wound care and antibiotics.';
    } else {
      riskLevel = 'High Risk';
      riskColor = 'bg-red-100 text-red-800';
      interpretation = 'High risk; aggressive infection prevention and monitoring required.';
    }
  ...
  return (
    <Box className="min-h-screen w-full bg-background text-foreground p-2">
      <Typography variant="h4" className="font-bold text-foreground mb-4">
        Sternal Infection Risk Calculator
      </Typography>
      <Box className="flex items-center mb-4">
        <Info className="w-5 h-5 text-primary mr-2" />
        <Typography variant="body2" className="text-muted-foreground">
          Estimates risk of sternal infection after cardiothoracic surgery. (Source: Gatti et al., J Thorac Cardiovasc Surg 2016)
        </Typography>
      </Box>
      ...
      <Box className="card w-full max-w-2xl mx-auto p-4 bg-card text-card-foreground shadow-lg rounded-radius">
        ...
        <Box className="flex gap-4">
          ...
          >
            Calculate Sternal Infection Risk
          </Button>
          <Button
            variant="outlined"
            onClick={handleReset}
          ...
          <Box className="mt-6 pt-4 border-t border-border">
            ...
            <Box className="flex items-center mt-2">
              ...
              <Typography variant="body2" className="text-muted-foreground">
                Source:{' '}
                <a
                  href="https://pubmed.ncbi.nlm.nih.gov/27045039/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-primary underline"
                >
                  Gatti et al., J Thorac Cardiovasc Surg 2016
                </a>
              </Typography>
            </Box>
          </Box>
...
export default SternalInfection;