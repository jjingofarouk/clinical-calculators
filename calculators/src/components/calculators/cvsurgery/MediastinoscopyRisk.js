...
const MediastinoscopyRisk = () => {
  ...
  const ranges = {
    ...
    tumorSize: { min: 0, max: 10, type: 'float', label: 'Tumor Size (cm)' },
  };
  ...
  const helperText = {
    ...
    tumorSize: 'Size of mediastinal tumor or lymph node (0-10 cm).',
  };
  ...
  const handleCalculate = () => {
    ...
    const coefficients = {
      ...
      performanceStatus: formData.performanceStatus * 0.25,
      tumorSize: formData.tumorSize > 3 ? 0.3 : 0,
    };
    ...
    if (score < 3) {
      ...
      interpretation = 'Low risk of complications; routine postoperative care recommended.';
    } else if (score <= 7) {
      riskLevel = 'Moderate Risk';
      riskColor = 'bg-yellow-100 text-yellow-800';
      interpretation = 'Moderate risk; consider enhanced monitoring and optimization.';
    } else {
      riskLevel = 'High Risk';
      riskColor = 'bg-red-100 text-red-800';
      interpretation = 'High risk; multidisciplinary consultation and careful planning required.';
    }
  ...
  return (
    <Box className="min-h-screen w-full bg-background text-foreground p-2">
      <Typography variant="h4" className="font-bold text-foreground mb-4">
        Mediastinoscopy Risk Calculator
      </Typography>
      <Box className="flex items-center mb-4">
        <Info className="w-5 h-5 text-primary mr-2" />
        <Typography variant="body2" className="text-muted-foreground">
          Estimates risk of complications for mediastinoscopy. (Source: Lemaire et al., Ann Thorac Surg 2005)
        </Typography>
      </Box>
      ...
      <Box className="card w-full max-w-2xl mx-auto p-4 bg-card text-card-foreground shadow-lg rounded-radius">
        ...
        <Box className="flex gap-4">
          ...
          >
            Calculate Mediastinoscopy Risk
          </Button>
          <Button
            variant="outlined"
            onClick={handleReset}
          ...
          <Box className="mt-6 pt-4 border-t border-border">
            ...
            <Box className="flex items-center mt-2">
              ...
              <Typography variant="body2" className="text-muted-foreground">
                Source:{' '}
                <a
                  href="https://pubmed.ncbi.nlm.nih.gov/15854928/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-primary underline"
                >
                  Lemaire et al., Ann Thorac Surg 2005;79:1552-1557
                </a>
              </Typography>
            </Box>
          </Box>
...
export default MediastinoscopyRisk;