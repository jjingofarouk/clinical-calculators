...
// Tamponade Risk Calculator Component
// Estimates risk of cardiac tamponade post-cardiac surgery
// Based on Kuvin et al. (Ann Thorac Surg 2002;73:781-786)
const TamponadeRisk = () => {
  ...
  const ranges = {
    ...
    bleeding: { min: 0, max: 1, type: 'integer', label: 'Significant Postoperative Bleeding (0 = No, 1 = Yes)' },
  ...
  const helperText = {
    ...
    bleeding: 'Significant postoperative bleeding requiring intervention (0 = No, 1 = Yes).',
  ...
  const handleCalculate = () => {
    ...
    if (score < 2) {
      ...
      interpretation = 'Low risk of tamponade; routine postoperative monitoring sufficient.';
    } else if (score <= 5) {
      riskLevel = 'Moderate Risk';
      riskColor = 'bg-yellow-100 text-yellow-800';
      interpretation = 'Moderate risk; consider echocardiography and close monitoring.';
    } else {
      riskLevel = 'High Risk';
      riskColor = 'bg-red-100 text-red-800';
      interpretation = 'High risk; urgent echocardiography and potential intervention required.';
    }
  ...
  return (
    <Box className="min-h-screen w-full bg-background text-foreground p-2">
      <Typography variant="h4" className="font-bold text-foreground mb-4">
        Tamponade Risk Calculator
      </Typography>
      <Box className="flex items-center mb-4">
        <Info className="w-5 h-5 text-primary mr-2" />
        <Typography variant="body2" className="text-muted-foreground">
          Estimates risk of cardiac tamponade following cardiac surgery. (Source: Kuvin et al., Ann Thorac Surg 2002)
        </Typography>
      </Box>
          ...
          <Box className="mt-6 pt-4 border-t border-border">
            ...
            <Box className="flex items-center mt-2">
              ...
              <Typography variant="body2" className="text-muted-foreground">
                Source:{' '}
                <a
                  href="https://pubmed.ncbi.nlm.nih.gov/11869988/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-primary underline"
                >
                  Kuvin et al., Ann Thorac Surg 2002;73:781-786
                </a>
              </Typography>
            </Box>
          </Box>
