...
// Esophagectomy Risk Calculator Component
// Estimates risk of mortality for esophagectomy
// Based on Steyerberg et al. (J Clin Oncol 2006;24:4347-4355)
const EsophagectomyRisk = () => {
  ...
  const ranges = {
    ...
    tumorStage: { min: 1, max: 4, type: 'integer', label: 'Tumor Stage (1-4)' },
    procedureType: { min: 1, max: 2, type: 'integer', label: 'Procedure Type (1 = Transhiatal, 2 = Transthoracic)' },
  };

  const helperText = {
    age: 'Patient age in years (18-100).',
    sex: 'Enter 0 for male, 1 for female.',
    performanceStatus: 'ECOG performance status (0 = fully active, 4 = completely disabled).',
    comorbidity: 'Number of major comorbidities (e.g., diabetes, COPD, heart disease; 0-3).',
    tumorStage: 'Tumor stage based on TNM classification (1-4).',
    procedureType: 'Type of esophagectomy: 1 = Transhiatal, 2 = Transthoracic.',
  };
  ...
  const handleCalculate = () => {
    ...
    const coefficients = {
      age: formData.age > 70 ? (formData.age - 70) * 0.04 : 0,
      sex: formData.sex === '1' ? 0.2 : 0,
      performanceStatus: formData.performanceStatus * 0.3,
      comorbidity: formData.comorbidity * 0.25,
      tumorStage: formData.tumorStage * 0.4,
      procedureType: formData.procedureType === '2' ? 0.5 : 0,
    };
    ...
    if (score < 5) {
      ...
      interpretation = 'Moderate risk; enhanced monitoring and nutritional support advised.';
    } else {
      riskLevel = 'High Risk';
  ...
  return (
    <Box className="min-h-screen w-full bg-background text-foreground p-2">
      <Typography variant="h4" className="font-bold text-foreground mb-4">
        Esophagectomy Risk Calculator
      </Typography>
      <Box className="flex items-center mb-4">
        <Info className="w-5 h-5 text-primary mr-2" />
        <Typography variant="body2" className="text-muted-foreground">
          Estimates risk of mortality for esophagectomy. (Source: Steyerberg et al., J Clin Oncol 2006)
        </Typography>
      </Box>
          ...
          <Box className="mt-6 pt-4 border-t border-border">
            ...
            <Box className="flex items-center mt-2">
              ...
              <Typography variant="body2" className="text-muted-foreground">
                Source:{' '}
                <a
                  href="https://pubmed.ncbi.nlm.nih.gov/16996927/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-primary underline"
                >
                  Steyerberg et al., J Clin Oncol 2006;24:4347-4355
                </a>
              </Typography>
            </Box>
          </Box>
...
export default EsophagectomyRisk;