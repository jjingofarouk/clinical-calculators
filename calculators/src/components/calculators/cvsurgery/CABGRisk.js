...
// CABG Risk Calculator Component
// Estimates risk of adverse outcomes for coronary artery bypass grafting
// Based on Hannan et al. (J Am Coll Cardiol 2006;47:1168-1174)
const CABGRisk = () => {
  ...
  const ranges = {
    ...
    renalFailure: { min: 0, max: 1, type: 'integer', label: 'Renal Failure (0 = No, 1 = Yes)' },
    ...
    vessels: { min: 1, max: 3, type: 'integer', label: 'Number of Diseased Vessels (1-3)' },
  };
  ...
  const helperText = {
    ...
    renalFailure: 'Renal failure requiring dialysis or creatinine >2 mg/dL (0 = No, 1 = Yes).',
    priorMI: 'History of myocardial infarction (0 = No, 1 = Yes).',
    urgency: 'Surgical urgency: 1 = Elective, 2 = Urgent, 3 = Emergency.',
    vessels: 'Number of coronary vessels with significant stenosis (1-3).',
  };
  ...
  const handleCalculate = () => {
    ...
    const coefficients = {
      age: formData.age > 65 ? (formData.age - 65) * 0.04 : 0,
      sex: formData.sex === '1' ? 0.5 : 0,
      lvef: formData.lvef < 50 ? 0.7 : 0,
      diabetes: formData.diabetes === '1' ? 0.4 : 0,
      renalFailure: formData.renalFailure === '1' ? 1.0 : 0,
      priorMI: formData.priorMI === '1' ? 0.3 : 0,
      urgency: [0, 0, 0.6, 1.2][formData.urgency],
      vessels: formData.vessels * 0.2,
    };

    const sum = Object.values(coefficients).reduce((a, b) => a + b, -4.0);
    ...
    if (score < 3) {
      ...
      interpretation = 'Low risk of adverse outcomes; standard postoperative care recommended.';
    } else if (score <= 7) {
      ...
      interpretation = 'High risk; consider multidisciplinary consultation and critical care planning.';
    }
  ...
  return (
    <Box className="min-h-screen w-full bg-background text-foreground p-2">
      <Typography variant="h4" className="font-bold text-foreground mb-4">
        CABG Risk Calculator
      </Typography>
      <Box className="flex items-center mb-4">
        <Info className="w-5 h-5 text-primary mr-2" />
        <Typography variant="body2" className="text-muted-foreground">
          Estimates risk of adverse outcomes following coronary artery bypass grafting. (Source: Hannan et al., J Am Coll Cardiol 2006)
        </Typography>
      </Box>
          ...
          <Box className="mt-6 pt-4 border-t border-border">
            ...
            <Box className="flex items-center mt-2">
              ...
              <Typography variant="body2" className="text-muted-foreground">
                Source:{' '}
                <a
                  href="https://pubmed.ncbi.nlm.nih.gov/16520068/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-primary underline"
                >
                  Hannan et al., J Am Coll Cardiol 2006;47:1168-1174
                </a>
              </Typography>
            </Box>
          </Box>
